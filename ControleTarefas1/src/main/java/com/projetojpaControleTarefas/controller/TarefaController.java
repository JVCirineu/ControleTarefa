package com.projetojpaControleTarefas.controller;
...
import com.projetojpaControleTarefas.service.TarefaService;
...
public class TarefaController {
	...
	@PostMapping
	public ResponseEntity<Tarefa> salvaTarefasControl(@RequestBody @Valid Tarefa tarefa){
		Tarefa salvaTarefa = tarefaService.salvaTarefa(tarefa);
		return ResponseEntity.status(HttpStatus.CREATED).body(salvaTarefa);
	}

	@PutMapping("/{id}")
	public ResponseEntity<Tarefa> alteraTarefaControlId(@PathVariable Long id,@RequestBody @Valid Tarefa tarefa ){
